...
Sebagai Technical Lead, berikut adalah analisis dan rekomendasi strategis untuk pengembangan LuhurCamp kedepannya agar lebih _scalable_, aman, dan user-friendly.
...
### Push Notifications (Firebase FCM) ✅ IMPLEMENTED
...
-   Reminder H-1 Check-in (TODO: Laravel Scheduler).

**Implementasi:**

-   Flutter: `notification_service.dart` - FCM initialization, local notifications, token sync
-   Laravel: `FCMService.php` - Send notifications via Firebase Admin SDK
-   Laravel: `FCMTokenController.php` - API endpoint untuk simpan/hapus FCM token
-   Integrasi: `BookingService.php` - Kirim notifikasi otomatis saat booking confirmed/rejected
